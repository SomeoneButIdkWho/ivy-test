...
class Greetings(commands.Cog):
    def __init__(self, bot):
        ...
        self.repeatable_greetings = ['sup', 'yo', 'hii', 'hiii', 'hai', 'hoi', 'hola', 'bonjour', 'como estas', 'ohayo', 'ohayo gozaimasu', 'hihi', 'hoihoi', 'pluh', 'cuh', 'yoo', 'yooo']
    ...
    async def on_message(self, message):
        ...
        for word in self.repeatable_greetings:
            if word in content_lower:
                repeatable_word = re.search(rf"\b{word}\b", content_lower).group()
                break

        if should_respond:
            if repeatable_word:
                if any(bot_name in content_lower for bot_name in self.bot_names):
                    if any(content_lower.startswith(bot_name) for bot_name in self.bot_names):
                ...
                return
            await message.channel.send(f"{random.choice(self.greetings)} {author_mention}")
