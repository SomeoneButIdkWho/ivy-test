...
class Purge(commands.Cog):
    ...
    async def confirm_action(self, ctx, count: int):
        ...
        msg = await ctx.send(
            f"⚠️ You are about to delete {count} messages. Confirm? ✅ / ❌")
        ...
        try:
            ...
            await ctx.send("⌛ Confirmation timed out. Purge cancelled.",
                           delete_after=5)
            return False
    ...
    async def purge(self, ctx, amount: int):
        # Only purge previous messages, don't delete invocation itself
        deleted_count = await self.safe_purge(ctx, amount)
        if deleted_count > 0:
            await ctx.send(f"✅ Deleted {deleted_count} messages.",
                           delete_after=5)
        else:
            await ctx.send(f"⚠️ No messages were deleted.", delete_after=5)

    # 👤 Purge by user
    @purge.command(name="user")
    @commands.has_permissions(manage_messages=True)
    async def purge_user(self, ctx, member: discord.Member, amount: int = 100):
        deleted_count = await self.safe_purge(
            ctx, amount, check=lambda m: m.author == member)
        if deleted_count > 0:
            await ctx.send(
                f"✅ Deleted {deleted_count} messages from {member.display_name}.",
                delete_after=5)
        else:
            await ctx.send(
                f"⚠️ No messages from {member.display_name} were deleted.",
                delete_after=5)
    ...
    async def purge_contains(self, ctx, *, keyword: str):
        ...
        if deleted_count > 0:
            await ctx.send(
                f"✅ Deleted {deleted_count} messages containing '{keyword}'.",
                delete_after=5)
        else:
            await ctx.send(
                f"⚠️ No messages containing '{keyword}' were deleted.",
                delete_after=5)
    ...
    async def purge_bots(self, ctx, amount: int = 100):
        deleted_count = await self.safe_purge(ctx,
                                              amount,
                                              check=lambda m: m.author.bot)
        if deleted_count > 0:
            await ctx.send(f"✅ Deleted {deleted_count} bot messages.",
                           delete_after=5)
        else:
            await ctx.send(f"⚠️ No bot messages were deleted.", delete_after=5)
    ...
    async def purge_humans(self, ctx, amount: int = 100):
        ...
        if deleted_count > 0:
            await ctx.send(f"✅ Deleted {deleted_count} human messages.",
                           delete_after=5)
        else:
            await ctx.send(f"⚠️ No human messages were deleted.",
                           delete_after=5)
    ...
    async def purge_attachments(self, ctx, amount: int = 100):
        deleted_count = await self.safe_purge(
            ctx, amount, check=lambda m: len(m.attachments) > 0)
        if deleted_count > 0:
            await ctx.send(
                f"✅ Deleted {deleted_count} messages with attachments.",
                delete_after=5)
        else:
            await ctx.send(f"⚠️ No messages with attachments were deleted.",
                           delete_after=5)
    ...
    async def purge_links(self, ctx, amount: int = 100):
        ...
        deleted_count = await self.safe_purge(
            ctx, amount, check=lambda m: bool(url_pattern.search(m.content)))
        if deleted_count > 0:
            await ctx.send(
                f"✅ Deleted {deleted_count} messages containing links.",
                delete_after=5)
        else:
            await ctx.send(f"⚠️ No messages containing links were deleted.",
                           delete_after=5)

    # ❌ Handle missing permissions and arguments
    @purge.error
    async def purge_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.send(
                "⚠️ Usage: `i!purge <amount>` or `i!purge <subcommand>`",
                delete_after=5)
